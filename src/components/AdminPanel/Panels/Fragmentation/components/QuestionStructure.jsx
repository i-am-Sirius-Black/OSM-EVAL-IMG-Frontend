...
const QuestionStructure = ({ questionGroups, setQuestionGroups, maxMarks }) => {
  ...
  const addSubquestion = (groupId) => {
    setQuestionGroups(questionGroups.map(group => {
      if (group.id === groupId) {
        ...
        // If this is the first subquestion, initialize choice-based settings
        const updatedGroup = {
          ...group,
          maxMarks: undefined, // Clear main question marks when adding parts
          subquestions: [
  ...
  // // Get the effective marks for a group (considering choice-based options)
  // const getEffectiveMarks = (group) => {
  //   if (group.subquestions.length === 0) {
  //     return parseFloat(group.maxMarks) || 0;
  //   } else if (group.choiceBased) {
  //     // For choice-based, show x out of y questions
  //     const subqTotal = group.subquestions.reduce(
  //       (sum, sq) => sum + (parseFloat(sq.maxMarks) || 0),
  //       0
  //     );
      
  //     // If all subquestions have the same marks, use that directly
  //     if (group.subquestions.every(sq => sq.maxMarks === group.subquestions[0].maxMarks)) {
  //       return group.choiceCount * (parseFloat(group.subquestions[0].maxMarks) || 0);
  //     }
