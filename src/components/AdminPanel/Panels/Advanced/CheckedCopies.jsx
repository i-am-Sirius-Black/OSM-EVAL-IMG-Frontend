...
//? v2 updated filtering removed session
...
const CheckedCopies = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if (response.data.filters) {
            setCourses(response.data.filters.courses || []);
            setSubjects(response.data.filters.subjects || []);
            setEvaluators(response.data.filters.evaluators || []);
          }
        } else {
          console.error('Failed to fetch checked copies');
        }
    ...
    fetchCheckedCopies();
  }, [filterEvaluator, filterCourse, filterSubject]);
  ...
  const handleDownloadAllCopies = async () => {
    ...
    try {
      ...
      // Download each copy and add to zip
      for (let i = 0; i < checkedCopies.length; i++) {
        const copy = checkedCopies[i];
        try {
          const response = await api.get(COPIES.DOWNLOAD_PDF(copy.copyId), {
            responseType: 'blob',
            timeout: 60000,
          ...
          // Update progress
          setDownloadProgress(Math.round(((i + 1) / checkedCopies.length) * 100));
        } catch (error) {
          console.error(`Error downloading copy ${copy.copyId}:`, error);
          // Continue with other copies
        }
        ...
        {loading ? (
          ...
          <div className="px-4 py-5 sm:py-1 sm:px-6">
            ...
            <div className="mb-4">
              <button
                onClick={handleDownloadAllCopies}
                disabled={downloadingAll || checkedCopies.length === 0}
                className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-gray-400"
              >
                {downloadingAll ? (
                  <>
                  ...
                  `Download All (${checkedCopies.length} copies)`
                )}
              </button>
            </div>
                        ...
                        {checkedCopies.length === 0 ? (
                          ...
                          checkedCopies.map((copy) => (
                            <tr key={copy.copyId} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900">{copy.evaluatorName || copy.evaluatorId}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900">{formatDateTime(copy.updatedAt)}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900">
                                 <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${copy.isReevaluated ? 'text-purple-800 bg-purple-100 ' : 'text-blue-800 bg-blue-100 '} mt-1`}>
                                    {copy.status}
                                  </span>
                                </div>
                                {/* {copy.isReevaluated && (
