...
export default function Evaluation() {
  ...
  useEffect(() => {
    ...
    const fetchSubjects = async () => {
      ...
      try {
        const response = await api.get(`/api/subjects/${selectedExam}`);
  ...
  const fetchCopies = async () => {
    ...
    try {
      ...
      if (subject) {
        ...
        // Fetch copies by subject ID
        const response = await api.post(`/api/copies/subject`, {
    ...
    const applyFiltersAndSort = async () => {
      ...
      try {
        ...
        if (selectedPackingId) {
          const response = await api.get("/api/copies/search", {
            params: {
        ...
        // Option 2: Client-side filtering (we'll fall back to this)
          ...
          // Fetch copies by subject again
          const response = await api.get(`/api/copies/subject/${selectedSubject}`);
