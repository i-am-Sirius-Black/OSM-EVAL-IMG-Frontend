...
//?v3.1 (testing) Fix for Page Scrolling Issue During Zoom
// import { memo, useState, useRef, useEffect, useCallback, useMemo } from "react";
// import SinglePage from "./SinglePage";
// import LoadingSpinner from "../Common/LoadingSpinner";
// import { getImageUrl } from "../../services/imageService";
...
//     // Add this code to preserve scroll position during zoom
...
//         // Calculate which part of the document is in the center of the viewport
//         const centerPosition = scrollTop + containerHeight / 2;

//         // Calculate relative position (0 to 1)
//         const scrollRatio = centerPosition / container.scrollHeight;

//         // Apply new zoom
//         setZoom(newZoom);

//         // After the DOM updates with new zoom level, restore the scroll position
//         setTimeout(() => {
//           if (containerRef.current) {
//             // Calculate new absolute position based on the same relative position
//             const newScrollTop =
//               containerRef.current.scrollHeight * scrollRatio -
//               containerHeight / 2;
...
//       // Replace all instances of setZoom with handleZoom
//       const zoomOut = () => handleZoom(Math.max(0.5, zoom - 0.1));
//       const zoomIn = () => handleZoom(Math.min(2, zoom + 0.1));
//       const resetZoom = () => handleZoom(0.8);

//       // Expose these functions to the component
//       containerRef.current.zoomIn = zoomIn;
//       containerRef.current.zoomOut = zoomOut;
//       containerRef.current.resetZoom = resetZoom;
//     }, [zoom]);
...
// // Replace your current handleZoom with this improved version:
...
//   // Performance optimization: Add a CSS transition for smoother zooming
//   const pages = container.querySelectorAll('.page-container');
//   pages.forEach(page => {
//     page.style.transition = 'width 0.2s ease-out, height 0.2s ease-out';
//   });
...
//   // After the DOM updates with new zoom level, restore the scroll position
//   // Increased timeout for more reliable scroll positioning
//   setTimeout(() => {
//     if (containerRef.current) {
//       // Calculate new absolute position based on the same relative position
//       const newScrollTop = (containerRef.current.scrollHeight * scrollRatio) - (containerHeight / 2);
//       containerRef.current.scrollTop = newScrollTop;
      
//       // Remove transitions after zoom is complete
//       setTimeout(() => {
//         pages.forEach(page => {
...
// // 2. Update your existing useEffect to use this shared function
// useEffect(() => {
//   // Expose these functions to the component
//   containerRef.current.zoomIn = () => handleZoom(Math.min(2, zoom + 0.1));
//   containerRef.current.zoomOut = () => handleZoom(Math.max(0.5, zoom - 0.1));
//   containerRef.current.resetZoom = () => handleZoom(0.8);
// }, [zoom, handleZoom]);

// // 3. Fix your wheel zoom handler to use the same handleZoom function
// useEffect(() => {
//   const handleWheel = (e) => {
//     if (e.ctrlKey) {
//       e.preventDefault(); // prevent browser default zoom
...
//       if (e.deltaY < 0) {
//         handleZoom(Math.min(2, zoom + 0.1));
//       } else {
//         handleZoom(Math.max(0.5, zoom - 0.1));
//       }
//     }
//   };
...
//           <div className="flex items-center bg-gray-50 rounded-md border border-gray-200 shadow-sm">
//             <button
//               onClick={() => containerRef.current.zoomOut()}
//               className="p-1 hover:bg-gray-100 text-gray-600"
//               title="Zoom out"
//             >
//               <svg className="h-4 w-4" viewBox="0 0 24 24">
...
//             </button>
//             <button
//               onClick={() => containerRef.current.zoomIn()}
//               className="p-1 hover:bg-gray-100 text-gray-600"
//               title="Zoom in"
//             >
//               <svg className="h-4 w-4" viewBox="0 0 24 24">
